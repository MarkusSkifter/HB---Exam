...
We solve and simulate two model variants: the baseline with $\tau = 0.3$ and a counterfactual with no labor income tax ($\tau = 0$). Following the exam specification, we set the cohort wage constants to $(\alpha_0,\ldots,\alpha_4) = (0.7, 0.8, 0.9, 1.1, 1.2)$ and draw initial human capital as $\tilde{k}_{i,0} \sim N(0.5, 1)$ (truncated to $[0,3]$). Both differ slightly from the starter code's defaults (a linear interpolation for $\alpha$ and $N(0,1)$ for $\tilde{k}_{i,0}$); we use the PDF values throughout.
\begin{figure}[H]
  \centering
